...
namespace Workflow.Tests.Groups
{
    public class GroupServiceTests : UmbracoHostTestBase
    {
        ...
        {
            _groupService.GroupCreated += (sender, args) =>
            {
                Assert.NotNull(args);
            };
        ...
        {
            const string description = "This is an updated";
        ...
        public async void Can_Delete_Group()
        {
            ...
            UserGroupPoco groupToDelete = groups.First();

            await _groupService.DeleteUserGroupAsync(groupToDelete.GroupId);

            IEnumerable<UserGroupPoco> groupsAfterDelete = await _groupService.GetUserGroupsAsync();

            // should be one less group
            Assert.True(originalCount > groupsAfterDelete.Count());

            // groupToDelete should not be returned
            Assert.True(!groupsAfterDelete.Any(g => g.GroupId == groupToDelete.GroupId));
        }
    }
}